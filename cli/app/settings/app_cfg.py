...
# source .env vars
fp_env = Path(DP_ROOT) / '.env'
fp_env_sample = Path(DP_ROOT) / '.env-sample'
if not Path(fp_env).exists():
  shutil.copyfile(fp_env_sample, fp_env)
  print(f'.env file auto-created at: {fp_env}')
load_dotenv(dotenv_path=fp_env, verbose=False)
...
LOGGER_NAME = 'SKYLIFT'
VERBOSITY = int(os.getenv('SKYLIFT_VERBOSITY'))
